package services
// เพิ่ม groupby function , join (แก้จากฟังก์ชั่น selectdata) 
import (
	"fmt"
	"strings"
...
// 	if where {
// 		query += " WHERE " + whereCon     // query = where id = 1,2,....
// 	} 
...
func SelectData(table string, fields []string, where bool, whereCon string, whereArgs []interface{}, groupBy string) ([]map[string]interface{}, error) {
	var query string = "SELECT "

	// เพิ่ม fields เข้าไปใน query
	for _, field := range fields {
		query += field + ", "
	}
	query = strings.TrimRight(query, ", ") // ลบ ", " ที่ท้ายสุด
	query += " FROM " + table

	// ตรวจสอบว่าต้องเพิ่ม WHERE ไหม
	if where && whereCon != "" {
		query += " WHERE " + whereCon
	}

	// ถ้ามี GROUP BY ให้เพิ่ม
	if groupBy != "" {
